...
cmake --build build -j 8 --target rsp_test save_test cheat_test archive_test wildmem_test rewind_test >/dev/null   || { echo "gate_all: los tests unitarios no compilan"; exit 1; }
for t in rsp_test save_test cheat_test archive_test wildmem_test rewind_test; do
